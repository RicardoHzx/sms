...
class Student extends React.Component {
  ...
  // 加载学生信息
  loadStudent(page=1,pageSize=8){
    this.setState({ loading:true})
    let url = "http://127.0.0.1:7979/student/pageQuery"
    $.get(url,{
      page,
      pageSize
    },({status,message,data})=>{
  ...
  batchDelete(){
    Modal.confirm({
      ...
      onOk:()=> {
        // 编写代码进行删除
        let url = "http://127.0.0.1:7979/student/batchDelete";
        $.ajax({
          url,
          method:"POST",
  ...
  toDelete =(id)=> {
    ...
    Modal.confirm({
      ...
      onOk:()=> {
        // 编写代码进行删除
        let url = "http://127.0.0.1:7979/student/deleteById?id="+id;
  ...
  handleOk = e => {
    ...
    this.form.validateFields((err, values) => {
      if (!err) {
        console.log(values)
        let url ="http://127.0.0.1:7979/student/saveOrUpdate";
      ...
      <div className="student">
        ...
        <div className="btns">
          ...
          <Button type="link" onClick={()=>{
            window.location.href="http://127.0.0.1:7979/student/download"
          }}>导出</Button>
        </div>
        {/* 表格 */}
        <Table 
